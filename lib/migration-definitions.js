...
function getMigrationDefinitions() {
  return [
    ...
    {
      name: '006_enhanced_features',
      sql: ENHANCED_FEATURES_SQL,
      checkFunction: async function() {
        const hasPaymentProofImage = await this.columnExists('deposit_requests', 'payment_proof_image');
        const hasCommissionEarned = await this.columnExists('referrals', 'commission_earned');
        const hasNewsletterImage = await this.columnExists('newsletters', 'image_url');
        return hasPaymentProofImage && hasCommissionEarned && hasNewsletterImage;
      }
    }
  ];
}
