...
export default function LiveTradeProgressCard({
  trade,
  onRefresh,
  ...
  const expectedTotalProfit =
    trade.amount * trade.hourly_profit_rate * trade.duration_hours;
  const profitPercentage =
    expectedTotalProfit > 0
      ? (trade.total_profit / expectedTotalProfit) * 100
      : 0;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Investment: ${formatCurrency(trade.amount)}
          </p>
        </div>
      ...
      {hourlyProfits.length > 0 && (
        <div className="mt-6 pt-6 border-t border-gray-200">
          ...
          <div className="text-sm text-gray-600">
            ...
            {hourlyProfits.length > 0
              ? new Date(
                  hourlyProfits[hourlyProfits.length - 1].created_at
                ).toLocaleString()
              : "None yet"}
          </div>
        </div>
